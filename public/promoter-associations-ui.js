(function () {
  ...
  function ensureStyle() {
    ...
    style.textContent = [
      ...
      ".apm-node-control{position:absolute;top:110px;right:14px;z-index:500;background:rgba(7,17,22,.92);border:1px solid rgba(255,255,255,.15);border-radius:12px;padding:10px;min-width:210px}",
      ".apm-node-control h4{margin:0 0 8px;font-size:11px;text-transform:uppercase;letter-spacing:.1em;color:#9deff7}",
      ".apm-node-control label{display:flex;align-items:center;gap:8px;font-size:13px;color:rgba(255,255,255,.86)}",
  ...
  function maybeInitMapAssociationLayer() {
    ...
    function ensureControl() {
      if (document.getElementById('apm-node-control')) return;
      var control = document.createElement('div');
      control.id = 'apm-node-control';
      control.className = 'apm-node-control';
      control.innerHTML = [
        '<h4>Tipo de nodo</h4>',
        '<label><input type="checkbox" data-apm-associations-layer checked> Asociaciones</label>',
        '<label style="margin-top:6px"><select data-apm-filter-entity class="apm-input" style="padding:6px 8px;font-size:12px"><option value="">Asociacion/Federacion/Gremio...</option><option value="association">Asociacion</option><option value="federation">Federacion</option><option value="guild">Gremio</option><option value="professional_organization">Organizacion profesional</option><option value="cooperative_network">Red cooperativa</option></select></label>',
        '<label style="margin-top:6px"><input data-apm-filter-country class="apm-input" style="padding:6px 8px;font-size:12px" placeholder="Pais (ES)"></label>',
        '<label style="margin-top:6px"><input data-apm-filter-community class="apm-input" style="padding:6px 8px;font-size:12px" placeholder="Comunidad autonoma"></label>',
        '<label style="margin-top:6px"><input data-apm-filter-province class="apm-input" style="padding:6px 8px;font-size:12px" placeholder="Provincia"></label>',
        '<label style="margin-top:6px"><input data-apm-filter-municipality class="apm-input" style="padding:6px 8px;font-size:12px" placeholder="Municipio"></label>',
        '<label style="margin-top:6px"><input data-apm-filter-craft class="apm-input" style="padding:6px 8px;font-size:12px" placeholder="Oficio relacionado (slug)"></label>',
        '<label style="margin-top:6px"><select data-apm-filter-verification class="apm-input" style="padding:6px 8px;font-size:12px"><option value="">Nivel de verificacion</option><option value="V1_SOURCE_FOUND">V1</option><option value="V2_ACTIVE">V2</option><option value="V3_IDENTITY_VALIDATED">V3</option><option value="V4_HISTORY_DOCUMENTED">V4</option><option value="V5_ENTITY_CONFIRMED">V5</option></select></label>',
        '<label style="margin-top:6px"><input type="checkbox" data-apm-filter-history> Entidades con historia documentada</label>',
        '<label style="margin-top:6px"><input type="checkbox" data-apm-filter-activities> Entidades con actividades/rutas/talleres</label>'
      ].join('');
      document.body.appendChild(control);
      var checkbox = control.querySelector('[data-apm-associations-layer]');
      if (checkbox) {
      ...
      function bindFilter(selector, key, transform) {
        var input = control.querySelector(selector);
        if (!input) return;
        input.addEventListener('change', function () {
          filterState[key] = transform ? transform(input) : String(input.value || '').trim();
          loadedIds.clear();
          layer.clearLayers();
          fetchByBounds();
        });
      ...
      bindFilter('[data-apm-filter-entity]', 'entity_type');
      bindFilter('[data-apm-filter-country]', 'country_code', function (input) { return String(input.value || '').trim().toUpperCase(); });
      bindFilter('[data-apm-filter-community]', 'autonomous_community');
      bindFilter('[data-apm-filter-province]', 'province');
      bindFilter('[data-apm-filter-municipality]', 'municipality');
      bindFilter('[data-apm-filter-craft]', 'craft_id');
      bindFilter('[data-apm-filter-verification]', 'verification_level');
      bindFilter('[data-apm-filter-history]', 'history_documented', function (input) { return input.checked; });
      bindFilter('[data-apm-filter-activities]', 'has_activities', function (input) { return input.checked; });
    }
